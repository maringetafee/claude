// Logica del agente "Responder" (compartida por responder.mjs y
// responder-background.mjs): pipeline en Netlify Blobs, llamada a Claude,
// aprendizaje de ventas cerradas/perdidas y precio dinamico.
//
...
// tasa de cierre por tipo de negocio, los motivos de perdida mas comunes y
// los precios vigentes.
//
// Precios, catalogo y reglas de venta: makemyweb.mjs.
...
export async function leerAprendizajes(store) {
  return (await store.get(APRENDIZAJES, { type: "json" })) || [];
}

export function calcularStats(lecciones) {
  const porTipo = {};
...
export function preciosVigentes(lecciones) {
...
// Ejecuta un analisis completo (llamada a Claude + guardado en el pipeline)
// y devuelve { resultado, fase, uso }. Tarda mas de 30 s, asi que se llama
// desde la funcion en segundo plano responder-background.mjs.
export async function ejecutarAnalisis(body) {
  const { slug, lead, canal = "whatsapp", conversacion = "", nota = "" } = body;
  if (!slugValido(slug)) throw new Error("slug invalido");
  if (!process.env.ANTHROPIC_API_KEY) throw new Error("Falta ANTHROPIC_API_KEY en las variables de entorno de Netlify");
  ...
  return { resultado, fase: ficha.fase, uso: msg.usage };
}

export async function guardarManual(body) {
